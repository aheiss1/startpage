...
class Element extends Node {
  ...
  renderContent (writer) {
    writer
      .write('<').write(html.escape(this._tag)).write(' ')
      .write(Object.entries(this._attributes).map(([name, value]) => html.escape(name) + "='" + html.escape(value) + "'").join(' '))
...
class List extends Body {
  mapToItems (values, mapper) {
    values.forEach(v => this.item(item => mapper(v, item)))
  }
}

class Table extends Body {
  mapToRows (values, mapper) {
    values.forEach(v => this.row(row => mapper(v, row)))
  }
}
...
class TableRow extends Body {
  ...
  _mapToCellElements (tag, values, mapper) {
    return this._addCellElements(tag, ...values.map(mapper))
  }

  mapToCells (values, mapper) {
    return this._mapToCellElements('td', values, mapper)
  }

  mapToHeadings (values, mapper) {
    return this._mapToCellElements('th', values, mapper)
  }
